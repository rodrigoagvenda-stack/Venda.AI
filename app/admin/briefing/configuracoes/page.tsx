...
export default function BriefingConfigPage() {
  ...
  // Estado para configuração de cadastro de cliente
  ...
  async function fetchClienteConfig() {
    try {
      ...
      if (!error.message?.includes('cliente_config')) {
        toast.error(error.message || 'Erro ao carregar configuracao do cliente');
      }
    } finally {
  ...
  async function handleClienteSave() {
    ...
    try {
      ...
      toast.success('Configuracao do cadastro de cliente salva com sucesso!');
      setClienteConfig(data.data);
    } catch (error: any) {
      console.error('Error saving cliente config:', error);
      toast.error(error.message || 'Erro ao salvar configuracao do cliente');
    } finally {
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Notificacoes quando CS cadastrar informacoes de cliente/empresa (/cliente)
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
